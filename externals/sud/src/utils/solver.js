...
export class Solver {
    #recorder
    #expertMode
    #simpleBacktrackingMode
    constructor(expertMode) {
        this.#expertMode = expertMode
    }

    #findNextEmpty(state) {
        if (this.#simpleBacktrackingMode) {
            for (let i = 0; i < 9; i++) {
                for (let j = 0; j < 9; j++) {
    ...
    #solve(state) {
        const [r, c] = this.#findNextEmpty(state)
        if (r === -1) {
    ...
    #checkIfSolutionStillPossible(state) {
        ...
        for (let i = 0; i < 27; i++) { // go through all groups
            if (state.counters[i].some(counter => counter === 0)) { // check if any counter in group smaller one
                return false
            }
        }
    ...
    #fixSquare(r, c, nr, state, from) {
        if (state.possibilities[r][c][nr - 1] === 0) {
            this.#recorder.record2(state, [-1], [`ERROR: Set sq(${r}, ${c}) to nr ${nr}`], { row: r, column: c })
            return false
        }
    ...
    solve({ givens, mode }, callback) {
        let isSolvable = false
        let isBacktracked = false
        let backtrackingIndex = -1

        let liveUpdate = true

        this.#simpleBacktrackingMode = mode
        this.#recorder = new Recorder(this.#expertMode, givens,
            () => {
                if (liveUpdate) {
                    callback(
                        [deepcopy(this.#recorder.timeline), isSolvable, isBacktracked,
                            backtrackingIndex, this.#unique, this.#firstSolutionIndex, true, this.#simpleBacktrackingMode]
                    )
                }
        ...
        const [r, c] = this.#findNextEmpty(state)
        isBacktracked = false
        backtrackingIndex = this.#recorder.timeline.length - 1
        if (r !== -1 && isSolvable) {
        ...
        return [deepcopy(this.#recorder.timeline), isSolvable, isBacktracked,
            backtrackingIndex, this.#unique, this.#firstSolutionIndex, false, this.#simpleBacktrackingMode]
