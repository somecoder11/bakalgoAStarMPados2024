function UnionFind() {
  class Node {
    ...
    drawBetweenChildrenNewNode(ctx, side_from_which_to_connect) {
      if(side_from_which_to_connect == "right")
      {
        ...
        ctx.lineTo(
          this.children[this.children.length - 1].getCoordsLeft()[0], 
          this.children[this.children.length - 1].getCoordsLeft()[1]
        )
        ctx.stroke();
    ...
    drawCurvedArrowTo(ctx, parent, strokeStyle)
    {
    ...
    drawArrowFromTo(ctx, storkeStyle="black") 
    {
      ctx.beginPath()
      const parentX = this.parent.getCoordsBottom()[0]
      const parentY = this.parent.getCoordsBottom()[1]
  ...
  class SquareNode extends Node{
    ...
    drawName(ctx) {
      ...
      if(this.parent === null)           
      {
        if(set_switch && switched_sets[1] === this.id)
        {
          ctx.font = "16px" + " " + fontFamily
          ctx.fillText("S  = S", this.x_center - 6, this.y_center - 2 * this.radius)
          const factor = (this.id).toString() === "10" ? 40 : 34
          const factor2 = (this.id).toString() === "10" ? 6 : 2
          const factor3 = (this.id).toString() === "10" ? 6 : 2
          ctx.fillText("," + (this.getTotalNumberOfChildren() + 1).toString(), this.x_center + factor, this.y_center - 2 * this.radius)
          ctx.font = "14px" + " " + fontFamily
          ctx.fillText((switched_sets[0]).toString(), this.x_center - this.radius + factor2, this.y_center - 2 * this.radius + 6)
          ctx.fillText((switched_sets[1]).toString(), this.x_center + this.radius + factor3, this.y_center - 2 * this.radius + 6)
        }
        else
        {
          ctx.font = "16px" + " " + fontFamily
          ctx.fillText("S", this.x_center - 6, this.y_center - 2 * this.radius)
          const factor = (this.id).toString() === "10" ? 24 : 18
          const factor2 = (this.id).toString() === "10" ? 7 : 5
          ctx.fillText("," + (this.getTotalNumberOfChildren() + 1).toString(), this.x_center + factor, this.y_center - 2 * this.radius)
          ctx.font = "14px" + " " + fontFamily
          ctx.fillText((this.id).toString(), this.x_center + factor2, this.y_center - 2 * this.radius + 6)
  ...
  let open_new_window_button = null
  ...
  let set_switch_step = -1;

  let switched_sets = [null, null]
  ...
  this.hightlightOpenNewWindowButton = function()
  {
    open_new_window_button.classList.add("active")
  }

  this.removeHighLightOpenNewWindowButtn = function() {
    open_new_window_button.classList.remove("active")
  }

  this.openNewWindow = async function()
  {
    if(animation_in_progress) return
    const features = 'width=1200,height=780,menubar=no,toolbar=no,location=no,status=no'
    // const dataURL = "data:text/html;charset=utf-8," + encodeURIComponent(customHTML)
    const new_window = window.open('temp/temp.html', "_blank", features)
    new_window.addEventListener("load", () => {
      new_window.postMessage([states_fast_find, states_fast_union, states_almost_linear, states_boxes, selected_sets , stepCount, commands, boxesStepCount, "openwindow",  switched_sets, set_switch_step], "*")
  })
    newWindow.push(new_window)
  }
  ...
  this.drawOperationBoxes = function(ctx, bottom_left_x, bottom_left_y)
  {
    ...
    const factor = 15
    let counter = 1;
    ctx.clearRect(bottom_left_x - 5, bottom_left_y + 5 + delta_y, 150, delta_y + 10)
  ...
  // Draw the right box where the operations are shown
  ...
  // Chose which parent sets are chosen depending on which button is currently active
  ...
  // Used for Fast Union and Almost linear to
  // return the child that the user is searching
  ...
  this.animateFindFastFind = async function(nodeToFind) 
  {
    ...
    // await new Promise(r => setTimeout(r, -1875 * speed + 1687.5))
  ...
  this.animateFindFastUnion = async function(node_to_find)
  {
    ...
    if(found_child === null) return null
    this.drawArrowFindFastUnion(context, found_child, true)
    await new Promise(r => setTimeout(r, -1500 * speed + 1650))
    return found_child
  }
  ...
  this.animateFindAlmostLinear = async function(node_to_find, draw)
  {
    ...
    await new Promise(r => setTimeout(r, -1875 * speed + 1937.5))
  ...
  this.drawArrowFindFastUnion = async function(ctx, node, draw)
  {
    ...
    while(node.parent !== null)
    {
      ...
      node = node.parent
      if(draw && node.parent !== null)
        await new Promise(r => setTimeout(r, -1600 * speed + 2160))
  ...
  this.findButtonFunction = async function() {
    if(animation_in_progress) return
    new_state = true
    animation_in_progress = true
    ...
    let added = false
    let to_search;
    to_search = prompt("Enter the element you want to find (letter): ")
    if(to_search === null)
    {
      animation_in_progress = false
      return
    }
    to_search = to_search.toUpperCase()
    if(newWindow !== null)
    newWindow.forEach((page) => {
      if(!page.closed)
      {
        page.postMessage([to_search, stepCount, boxesStepCount, "find"], "*")
      }
    })
    ...
    unionFind.updateCoordsCurrentSelectorsTree(unionFind.sortedParents(parent_nodes_fast_union), states_fast_union)
    unionFind.updateCoordsCurrentSelectorsTree(unionFind.sortedParents(parent_nodes_almost_linear), states_almost_linear)
    // new_state = false;
    // await unionFind.redrawCanvas();
    // new_state = true;
  }

  this.checkInputValidity = function(input)
  {
    if(input === null)
    {
      return false
    ...
    return !isNaN(number) && 1 <= number && number <= 10
  }
  ...
  this.initialize = function() 
  {
    window.addEventListener('message', () => {
      ...
      const buttonNodes = document.getElementById("numNodesButton")
      const buttonUnion = document.getElementById("unionButton")
      const findButton = document.getElementById("findButton")
      open_new_window_button = document.getElementById("new_window_button")
    ...
    findButton.addEventListener('click', this.findButtonFunction)


    buttonNodes.addEventListener('click', () =>{
      let input = prompt("Please enter the number of nodes you wish to initialize:")
      if(this.checkInputValidity(input)) {
          if(parent_nodes_fast_find.length != 0 || selected_circles_fast_find.length != 0 || parent_nodes_fast_union.length != 0) // || child_nodes_fast_find.length != 0)
          {
          ...
          commands = [[]]
          current_boxes = []
          const n = parseInt(Number(input), 10)
          stepCount = 0
          boxesStepCount = 0
          const arrowLength = min_object_distance

          this.putCanvasIntoLittleSquares(n)

          for(let i = 0; i < n; i++)
          {
            
            const square_radius = 5/6 * radius
            // const square_radius = radius

            var par_fast_find = new CircleNode((i + 1).toString(10), [0, 0], square_radius, i + 1)
            var par_fast_union = new SquareNode(String.fromCharCode(65 + i), null, [0, 0], square_radius, i + 1)
            var par_almost_linear = new SquareNode(String.fromCharCode(65 + i), null, [0, 0], square_radius, i + 1)
            parent_nodes_fast_union.push(par_fast_union)
            parent_nodes_fast_find.push(par_fast_find)
            parent_nodes_almost_linear.push(par_almost_linear)
          

            var child = new SquareNode(String.fromCharCode(65 + i), par_fast_find, [0, 0], square_radius, i + 1)
            par_fast_find.addChild(child)
          }

          if(newWindow !== null)
          newWindow.forEach((page) => {
            page.postMessage([parent_nodes_fast_find, parent_nodes_fast_union, parent_nodes_almost_linear, current_boxes, "nodesButton"], "*")
          })
      }
      else
      {
        alert(`Please enter a number between 1 and ${max_number_of_nodes}`)
      }
  ...
  // check if selector has been clicked
  canvas.addEventListener("click", function(event) 
  {
    ...
    if(active_button_number === 1)
    {
      unionFind.highlightClickedSet(parent_nodes_fast_find, mouseX, mouseY)
    }
    else if(active_button_number === 2)
    {
      unionFind.highlightClickedSet(parent_nodes_fast_union, mouseX, mouseY)
    }
    else if(active_button_number === 3)
    {
      unionFind.highlightClickedSet(parent_nodes_almost_linear, mouseX, mouseY)
    }
    newWindow.forEach((page) =>{
      page.postMessage([selected_sets, stepCount, boxesStepCount, "selectedSet"], "*")
    })
    unionFind.redrawCanvas()
    new_state = true
  })
  ...
  this.highlightClickedSet = function(set, x, y)
  {
    for(let i = 0; i < set.length; i++)
    {
      const selector = set[i]
      if(selector == undefined) return
      const distance = Math.sqrt((x - selector.getCoordsCenter()[0]) ** 2 + (y - selector.getCoordsCenter()[1]) ** 2)
      if(distance <= selector.radius)
      {
        if(selected_sets[0] == selector)
        {
          selected_sets.splice(0, 1)
        }
        else if(selected_sets[1] == selector)
        {
          selected_sets.splice(1, 1)
        }
        else if(selected_sets.length < 2)
        {
          selected_sets.push(selector)
        }
        else
        {
          selected_sets.splice(0, 1)
  ...
  this.determineSetWithLessChildrenTree = function(selected_circles_list)
  {
    let parent_with_less_children;
    if(selected_circles_list[0].getTreeHeight() > selected_circles_list[1].getTreeHeight())
    {
      parent_with_less_children = selected_circles_list[1]
    }
    else if(selected_circles_list[0].getTreeHeight() < selected_circles_list[1].getTreeHeight())
    {
      parent_with_less_children = selected_circles_list[0]
    }
    else
    {
      if(selected_circles_list[0].id < selected_circles_list[1].id)
      {
        parent_with_less_children = selected_circles_list[1]
      }
      else
    ...
    var index_of_parent_with_less_children_selected_sets = selected_circles_list.indexOf(parent_with_less_children)
  ...
  // Returns the index of the parent with less children
  this.determineSetWithLessChildrenFastFind = function()
  {
    let parent_with_less_children;
  ...
  this.unionButtonFunction = async function() {
    ...
    {
      alert("Please first select two sets to union, by clicking on then.")
      return
    }
    new_state = true
    ...
    let index_of_parent_with_less_children_fast_find = await unionFind.determineSetWithLessChildrenFastFind()
    let index_of_parent_with_more_children_fast_find = index_of_parent_with_less_children_fast_find ? 0 : 1

    let index_of_parent_with_less_children_fast_union = await unionFind.determineSetWithLessChildrenTree(selected_circles_fast_union)
    let index_of_parent_with_more_children_fast_union = index_of_parent_with_less_children_fast_union ? 0 : 1

    let index_of_parent_with_less_children_almost_linear = await unionFind.determineSetWithLessChildrenTree(selected_circles_almost_linear)
    let index_of_parent_with_more_children_almost_linear = index_of_parent_with_less_children_almost_linear ? 0 : 1

    if(index_of_parent_with_less_children_almost_linear !== index_of_parent_with_less_children_fast_find)
    {
      set_switch = true;
      set_switch_step = stepCount;
      let id_less = selected_circles_fast_find[index_of_parent_with_less_children_fast_find].id
      let id_more = selected_circles_fast_find[index_of_parent_with_more_children_fast_find].id
      switched_sets[0] = id_less
      switched_sets[1] = id_more
      switched_sets = [id_less, id_more]
      selected_circles_fast_union[index_of_parent_with_less_children_fast_union].id = id_less
      selected_circles_fast_union[index_of_parent_with_more_children_fast_union].id = id_more
      selected_circles_almost_linear[index_of_parent_with_less_children_almost_linear].id = id_less
      selected_circles_almost_linear[index_of_parent_with_more_children_almost_linear].id = id_more
    }
    ...
    var side_from_which_to_connect = "right"
      ...
      animation_in_progress = true
      newWindow.forEach((page) =>{
        page.postMessage([stepCount - 1, boxesStepCount, "union"], "*")
      })
      await unionFind.animateFastFind(context, selected_circles_fast_find[index_of_parent_with_less_children_fast_find], selected_circles_fast_find[index_of_parent_with_more_children_fast_find], side_from_which_to_connect, active_button_number === 1)
      await unionFind.animateFastUnion(context, selected_circles_fast_union[index_of_parent_with_less_children_fast_union], selected_circles_fast_union[index_of_parent_with_more_children_fast_union], side_from_which_to_connect, active_button_number === 2)
      await unionFind.animateAlmostLinear(context, selected_circles_almost_linear[index_of_parent_with_less_children_almost_linear], selected_circles_almost_linear[index_of_parent_with_more_children_almost_linear], side_from_which_to_connect, active_button_number === 3)
        
      selected_circles_fast_find[index_of_parent_with_more_children_fast_find].number_of_members += selected_circles_fast_find[index_of_parent_with_less_children_fast_find].number_of_members
      selected_circles_fast_union[index_of_parent_with_more_children_fast_union].number_of_members += selected_circles_fast_union[index_of_parent_with_less_children_fast_union].number_of_members
      selected_circles_almost_linear[index_of_parent_with_more_children_almost_linear].number_of_members += selected_circles_almost_linear[index_of_parent_with_less_children_almost_linear].number_of_members
  ...
  this.animateFastUnion = async function(ctx, par_with_less, par_with_more, side_from_which_to_connect, draw) 
  {
    ...
    //STEP1
    //SET NEW PARENT
    par_with_more.addChild(par_with_less)
    // par_with_less.setParent(par_with_more)
    
    
    if(draw)
    {
      par_with_less.drawCurvedArrowTo(ctx, par_with_more, "green")
      await new Promise(r => setTimeout(r, -1000 * speed + 1600 ))
    }
    par_with_more.number_of_members += par_with_less.number_of_members
  ...
  this.connectTreeArrows = function(ctx, root, algorithm, exception = null, side_from_which_to_connect = null, color="green") 
  {
    ...
    else 
    {
      ...
      for(let i = 0; i < num_of_children; i++)
      {
        ...
        this.connectTreeArrows(ctx, root.children[i], "almost_linear", exception, side_from_which_to_connect, color)
      }
    }
  }
  ...
  this.drawCurrentNodesSelected = function(ctx, strokeColor, fillColor) {
      ...
      // have if here for which algo is currently selected
      if(active_button_number == 1)
      {
          this.drawNodesFastFind(ctx)
      } 
  ...
  this.drawBoxes = function() {
      ...
      current_boxes.forEach( square => {
          ...
          // if(i)
          // {
          //   cont.strokeRect(topLeftX, topLeftY, width, height / 3)
          //   cont.strokeRect(topLeftX, topLeftY, width, height * 2 / 3)
          //   cont.strokeRect(topLeftX, topLeftY, width / 4, height)
          //   cont.strokeRect(topLeftX, topLeftY, width / 2, height)
          //   cont.strokeRect(topLeftX, topLeftY, width * 3 / 4, height)
          //   i = false
          // }
        })
  }
  ...
  // Used for Fast Union and Almost Linear
  ...
  this.animateStepForward = async function()
  {
    ...
    states_fast_find[stepCount - 1].forEach(s => {
      ...
      newWindow.forEach((page) =>{
        let temp = boxesStepCount
        // if(commands[stepCount - 1][1] === "union")
        //   temp--
        page.postMessage([stepCount - 1, temp, "stepforward"], "*")
      })
  ...
  this.stepForward = function() {
    if(stepCount === states_fast_find.length - 1 || animation_in_progress || !states_fast_find.length) return;
    animation_in_progress = true
    new_state = false
    stepCount++;
    let context = canvas.getContext("2d");
  ...
  this.stepBackward = function() {
    if(stepCount <= 0 || animation_in_progress || !states_fast_find.length) return;
    new_state = false
    if(commands[stepCount][1] === "union")
      boxesStepCount--
    stepCount--;
    if(stepCount === set_switch_step - 1)
    {
      set_switch = false;
    }
    let context = canvas.getContext("2d");
    ...
    newWindow.forEach((page) =>{
      ...
      if(commands[stepCount + 1][1] === "union")
        temp++
      page.postMessage([stepCount + 1, temp, "stepbackward"], "*")
    })
    selected_sets = [null, null]
    new_state = false
    this.redrawCanvas()
  ...
  this.updateSpeed = function(new_speed) {
    newWindow.forEach(page => {
      page.postMessage(["new_speed",new_speed, stepCount, boxesStepCount], "*")
    })
  ...
  this.randomizeStart = async function() {
    // const input = document.getElementById("numNodesInput")
    // const buttonNodes = document.getElementById("numNodesButton")
    // const buttonUnion = document.getElementById("unionButton")
    // const findButton = document.getElementById("findButton")
    // input.value = randomIntFromInterval(2, 10);
    // buttonNodes.click()

    let c = 0
    while(c < 15)
    {
      let delete_later = String.fromCharCode(65 + (c % 5))
      await this.findButtonFunction(delete_later)
      c++
    }
  }
  ...
  this.fastForward = function() 
  {
    ...
    if(animation_in_progress || states_fast_find.length == 1) return
  ...
  this.fastBackward = function()
  {
    ...
    if(animation_in_progress || 1 == states_fast_find.length) return
    while(stepCount > 0)
    {
      if(commands[stepCount][1] === "union")
      {
